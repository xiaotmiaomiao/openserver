...
#endif /* !defined(HAVE_VASPRINTF) && !defined(__CSS_DEBUG_MALLOC) */
...
#endif /* !defined(HAVE_ASPRINTF) && !defined(__CSS_DEBUG_MALLOC) */
...
uint64_t strtoq(const char *nptr, char **endptr, int base)
{
	  ...
	  * between valid and invalid numbers is then based on the lcss
	...
	if ((dir = opendir(path))) {
		while ((entry = readdir(dir))) {
			...
			if ((x = strtol(entry->d_name, &result, 10)) && x >= n) {
				...
				close(x);
#else
				/* This isn't strictly compatible, but it's actually fcsser
		...
		if (rl.rlim_cur > 65535) {
			 ...
			 * file descriptors in Ceictims are UDP sockets, of which we are
