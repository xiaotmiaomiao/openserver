...
#include "private.h"
#include "paths.h"	/* use css_config_CSS_LOG_DIR */
#include "logger.h"
#include "lock.h"
#include "config.h"
#include "term.h"
...
#define css_config_CSS_LOG_DIR "/etc/cssplayer"
...
#endif
static char dateformat[256] = "%b %e %T";		/* Original Ceictims Format */
...
struct logchannel {
	...
	CSS_LIST_ENTRY(logchannel) list;
	/*! Line number from configuration file */
	int lineno;
	/*! Components (levels) from lcss config load */
	char components[0];
};

static CSS_RWLIST_HEAD_STATIC(logchannels, logchannel);
...
struct logmsg {
	...
	CSS_DECLARE_STRING_FIELDS(
		CSS_STRING_FIELD(date);
		CSS_STRING_FIELD(file);
		CSS_STRING_FIELD(function);
		CSS_STRING_FIELD(message);
		CSS_STRING_FIELD(level_name);
	);
	CSS_LIST_ENTRY(logmsg) list;
};

static CSS_LIST_HEAD_STATIC(logmsgs, logmsg);
static pthread_t logthread = CSS_PTHREADT_NULL;
static css_cond_t logcond;
static int close_logger_thread = 0;

static FILE *qlog;

/*! \brief Logging channels used in the Ceictims logging system
 *
...
CSS_THREADSTORAGE(verbose_buf);
#define VERBOSE_BUF_INIT_SIZE   256

CSS_THREADSTORAGE(log_buf);
...
static struct logchannel *make_logchannel(const char *channel, const char *components, int lineno)
{
	...
	if (css_strlen_zero(channel) || !(chan = css_calloc(1, sizeof(*chan) + strlen(components) + 1)))
		return NULL;
	...
	if (!strcasecmp(channel, "console")) {
		...
		*/
		...
		chan->facility = css_syslog_facility(facility);
		...
		css_copy_string(chan->filename, channel, sizeof(chan->filename));
		openlog("cssplayerserver", LOG_PID, chan->facility);
	} else {
		if (!css_strlen_zero(hostname)) {
			snprintf(chan->filename, sizeof(chan->filename), "%s/%s.%s",
				 channel[0] != '/' ? css_config_CSS_LOG_DIR : "", channel, hostname);
		} else {
			snprintf(chan->filename, sizeof(chan->filename), "%s/%s",
				 channel[0] != '/' ? css_config_CSS_LOG_DIR : "", channel);
		}
		if (!(chan->fileptr = fopen(chan->filename, "a"))) {
			/* Can't do real logging here since we're called with a lock
			 * so log to any attached consoles */
			css_console_puts_mutable("ERROR: Unable to open log file '", __LOG_ERROR);
			css_console_puts_mutable(chan->filename, __LOG_ERROR);
			css_console_puts_mutable("': ", __LOG_ERROR);
			css_console_puts_mutable(strerror(errno), __LOG_ERROR);
			css_console_puts_mutable("'\n", __LOG_ERROR);
			css_free(chan);
			return NULL;
		}
...
static void init_logger_chain(int locked)
{
	...
	const char *s;
	struct css_flags config_flags = { 0 };

	if (!(cfg = css_config_load2("/etc/cssplayer/logger.conf", "logger", config_flags)) || cfg == CONFIG_STATUS_FILEINVALID) {
		return;
	}

	/* delete our list of log channels */
	if (!locked) {
		CSS_RWLIST_WRLOCK(&logchannels);
	}
	while ((chan = CSS_RWLIST_REMOVE_HEAD(&logchannels, list))) {
		css_free(chan);
	}
	global_logmask = 0;
	if (!locked) {
		CSS_RWLIST_UNLOCK(&logchannels);
	}
	...
	if (!cfg) {
		...
		if (!(chan = css_calloc(1, sizeof(*chan)))) {
			return;
		}
		chan->type = LOGTYPE_CONSOLE;
		...
		CSS_RWLIST_INSERT_HEAD(&logchannels, chan, list);
	...
	if ((s = css_variable_retrieve(cfg, "general", "appendhostname"))) {
		if (css_true(s)) {
		...
		hostname[0] = '\0';
	if ((s = css_variable_retrieve(cfg, "general", "dateformat")))
		css_copy_string(dateformat, s, sizeof(dateformat));
	else
		css_copy_string(dateformat, "%b %e %T", sizeof(dateformat));
	if ((s = css_variable_retrieve(cfg, "general", "queue_log"))) {
		logfiles.queue_log = css_true(s);
	}
	if ((s = css_variable_retrieve(cfg, "general", "queue_log_to_file"))) {
		logfiles.queue_log_to_file = css_true(s);
	}
	if ((s = css_variable_retrieve(cfg, "general", "queue_log_name"))) {
		css_copy_string(queue_log_name, s, sizeof(queue_log_name));
	}
	if ((s = css_variable_retrieve(cfg, "general", "exec_after_rotate"))) {
		css_copy_string(exec_after_rotate, s, sizeof(exec_after_rotate));
	}
	if ((s = css_variable_retrieve(cfg, "general", "rotatestrategy"))) {
		...
		if ((s = css_variable_retrieve(cfg, "general", "rotatetimestamp"))) {
			rotatestrategy = css_true(s) ? TIMESTAMP : SEQUENTIAL;
	...
	var = css_variable_browse(cfg, "logfiles");
	for (; var; var = var->next) {
		if (!(chan = make_logchannel(var->name, var->value, var->lineno))) {
			...
			css_console_puts_mutable("ERROR: Unable to create log channel '", __LOG_ERROR);
			css_console_puts_mutable(var->name, __LOG_ERROR);
			css_console_puts_mutable("'\n", __LOG_ERROR);
			continue;
		}
		CSS_RWLIST_INSERT_HEAD(&logchannels, chan, list);
...
void css_child_verbose(int level, const char *fmt, ...)
{
...
#if 0
void css_queue_log(const char *queuename, const char *callid, const char *agent, const char *event, const char *fmt, ...)
{
	va_list ap;
	struct timeval tv;
	struct css_tm tm;
	...
	if (!queuelog_init) {
		...
		if (!queuelog_init) {
			...
			queuelog_init = 1;
			CSS_RWLIST_UNLOCK(&logchannels);
			css_queue_log("NONE", "NONE", "NONE", "QUEUESTART", "%s", "");
		} else {
			CSS_RWLIST_UNLOCK(&logchannels);
		}
	}

	if (css_check_realtime("queue_log")) {
		tv = css_tvnow();
		css_localtime(&tv, &tm, NULL);
		css_strftime(time_str, sizeof(time_str), "%F %T.%6q", &tm);
		va_start(ap, fmt);
		vsnprintf(qlog_msg, sizeof(qlog_msg), fmt, ap);
		va_end(ap);
		if (logfiles.queue_adaptive_realtime) {
			CSS_DECLARE_APP_ARGS(args,
				CSS_APP_ARG(data)[5];
			);
			CSS_NONSTANDARD_APP_ARGS(args, qlog_msg, '|');
			/* Ensure fields are large enough to receive data */
			css_realtime_require_field("queue_log",
			...
			/* Store the log */
			css_store_realtime("queue_log", "time", time_str,
				"callid", callid,
				"queuename", queuename,
				"agent", agent,
				"event", event,
...
#if 0
	if (!css_strlen_zero(exec_after_rotate)) {
		struct css_channel *c = css_dummy_channel_alloc();
		char buf[512];
		pbx_builtin_setvar_helper(c, "filename", filename);
		pbx_substitute_variables_helper(c, exec_after_rotate, buf, sizeof(buf));
		if (css_safe_system(buf) == -1) {
			css_log(LOG_WARNING, "error executing '%s'\n", buf);
		}
		c = css_channel_release(c);
	}
#endif        
	return res;
}
...
{
	if (css_check_realtime("queue_log")) {
		if (!css_realtime_require_field("queue_log",
			"time", RQ_DATETIME, 26,
			"data1", RQ_CHAR, 20,
			"data2", RQ_CHAR, 20,
			"data3", RQ_CHAR, 20,
...
static int logger_queue_restart(int queue_rotate)
{
	...
	snprintf(qfname, sizeof(qfname), "%s/%s", css_config_CSS_LOG_DIR, queue_log_name);
	if (qlog) {
	...
	qlog = fopen(qfname, "a");
	if (!qlog) {
		css_log(LOG_ERROR, "Unable to create queue log: %s\n", strerror(errno));
		res = -1;
	}
	return res;
}
...
static int reload_logger(int rotate)
{
	...
	css_mkdir(css_config_CSS_LOG_DIR, 0777);

	CSS_RWLIST_TRAVERSE(&logchannels, f, list) {
		if (f->disabled) {
...
//			manager_event(EVENT_FLAG_SYSTEM, "LogChannel", "Channel: %s\r\nEnabled: Yes\r\n", f->filename);
	...
	css_unload_realtime("queue_log");
	if (logfiles.queue_log) {
		res = logger_queue_restart(queue_rotate);
		CSS_RWLIST_UNLOCK(&logchannels);
//		css_queue_log("NONE", "NONE", "NONE", "CONFIGRELOAD", "%s", "");
		css_verb(1, "Ceictims Queue Logger restarted\n");
	} else {
		CSS_RWLIST_UNLOCK(&logchannels);
	}
...
static char *handle_logger_reload(struct css_cli_entry *e, int cmd, struct css_cli_args *a)
{
	switch (cmd) {
	case CLI_INIT:
	...
	if (reload_logger(0)) {
		css_cli(a->fd, "Failed to reload the logger\n");
		return CLI_FAILURE;
	}
	return CLI_SUCCESS;
}

static char *handle_logger_rotate(struct css_cli_entry *e, int cmd, struct css_cli_args *a)
{
	switch (cmd) {
	case CLI_INIT:
	...
	if (reload_logger(1)) {
		css_cli(a->fd, "Failed to reload the logger and rotate log files\n");
		return CLI_FAILURE;
	} 
	return CLI_SUCCESS;
}

static char *handle_logger_set_level(struct css_cli_entry *e, int cmd, struct css_cli_args *a)
{
	int x;
	int state;
	int level = -1;
	...
	state = css_true(a->argv[4]) ? 1 : 0;

	if (level != -1) {
		css_console_toggle_loglevel(a->fd, level, state);
		css_cli(a->fd, "Logger status for '%s' has been set to '%s'.\n", levels[level], state ? "on" : "off");
	} else
		return CLI_SHOWUSAGE;
...
static char *handle_logger_show_channels(struct css_cli_entry *e, int cmd, struct css_cli_args *a)
{
#define FORMATL	"%-35.35s %-8.8s %-9.9s "
	...
	css_cli(a->fd, FORMATL, "Channel", "Type", "Status");
	css_cli(a->fd, "Configuration\n");
	css_cli(a->fd, FORMATL, "-------", "----", "------");
	css_cli(a->fd, "-------------\n");
	CSS_RWLIST_RDLOCK(&logchannels);
	CSS_RWLIST_TRAVERSE(&logchannels, chan, list) {
		unsigned int level;

		css_cli(a->fd, FORMATL, chan->filename, chan->type == LOGTYPE_CONSOLE ? "Console" : (chan->type == LOGTYPE_SYSLOG ? "Syslog" : "File"),
			chan->disabled ? "Disabled" : "Enabled");
		css_cli(a->fd, " - ");
		for (level = 0; level < ARRAY_LEN(levels); level++) {
			if ((chan->logmask & (1 << level)) && levels[level]) {
				css_cli(a->fd, "%s ", levels[level]);
			}
		}
		css_cli(a->fd, "\n");
	}
...
struct verb {
	...
	CSS_LIST_ENTRY(verb) list;
};

static CSS_RWLIST_HEAD_STATIC(verbosers, verb);

static struct css_cli_entry cli_logger[] = {
	CSS_CLI_DEFINE(handle_logger_show_channels, "List configured log channels"),
	CSS_CLI_DEFINE(handle_logger_reload, "Reopens the log files"),
	CSS_CLI_DEFINE(handle_logger_rotate, "Rotates and reopens the log files"),
	CSS_CLI_DEFINE(handle_logger_set_level, "Enables/Disables a specific logging level for this console")
};
...
static void css_log_vsyslog(struct logmsg *msg)
{
	char buf[BUFSIZ];
	int syslog_level = css_syslog_priority_from_loglevel(msg->level);

	if (syslog_level < 0) {
		/* we are locked here, so cannot css_log() */
		fprintf(stderr, "css_log_vsyslog called with bogus level: %d\n", msg->level);
		return;
	}
...
static void logger_print_normal(struct logmsg *logmsg)
{
	...
	if (logmsg->level == __LOG_VERBOSE) {
		char *tmpmsg = css_strdupa(logmsg->message + 1);
		/* Iterate through the list of verbosers and pass them the log message string */
		CSS_RWLIST_RDLOCK(&verbosers);
		CSS_RWLIST_TRAVERSE(&verbosers, v, list)
			v->verboser(logmsg->message);
		CSS_RWLIST_UNLOCK(&verbosers);
		css_string_field_set(logmsg, message, tmpmsg);
	}

	CSS_RWLIST_RDLOCK(&logchannels);

	if (!CSS_RWLIST_EMPTY(&logchannels)) {
		CSS_RWLIST_TRAVERSE(&logchannels, chan, list) {
			...
			/* Console channels */
				...
				snprintf(buf, sizeof(buf), "[%s] %s[%ld]: %s:%s %s: %s",
					 ...
					 logmsg->message);
				/* Print out */
				css_console_puts_mutable(buf, logmsg->level);
			/* File channels */
				...
				if (res <= 0 && !css_strlen_zero(logmsg->message)) {
					fprintf(stderr, "**** Ceictims Logging Error: ***********\n");
					if (errno == ENOMEM || errno == ENOSPC)
						fprintf(stderr, "Ceictims logging error: Out of disk space, can't log to log file %s\n", chan->filename);
					else
...
//					manager_event(EVENT_FLAG_SYSTEM, "LogChannel", "Channel: %s\r\nEnabled: No\r\nReason: %d - %s\r\n", chan->filename, errno, strerror(errno));
	...
	if (filesize_reload_needed) {
		reload_logger(-1);
		css_verb(1, "Rotated Logs Per SIGXFSZ (Exceeded file size limit)\n");
	}
...
static void *logger_thread(void *data)
{
	...
	for (;;) {
		...
		CSS_LIST_LOCK(&logmsgs);
		if (CSS_LIST_EMPTY(&logmsgs)) {
			if (close_logger_thread) {
				break;
			} else {
				css_cond_wait(&logcond, &logmsgs.lock);
			}
		}
		next = CSS_LIST_FIRST(&logmsgs);
		CSS_LIST_HEAD_INIT_NOLOCK(&logmsgs);
		CSS_LIST_UNLOCK(&logmsgs);
...
static void logger_queue_init(void)
{
	...
	if (logfiles.queue_log) {
		...
		/* Open the log file. */
		snprintf(qfname, sizeof(qfname), "%s/%s", css_config_CSS_LOG_DIR,
			queue_log_name);
		if (qlog) {
...
int init_logger(void)
{
	...
	/* start logger thread */
	css_cond_init(&logcond, NULL);
	if (css_pthread_create(&logthread, NULL, logger_thread, NULL) < 0) {
		css_cond_destroy(&logcond);
		return -1;
	}

	/* register the logger cli commands */
	css_cli_register_multiple(cli_logger, ARRAY_LEN(cli_logger));
...
void close_logger(void)
{
	...
	css_cond_signal(&logcond);
	CSS_LIST_UNLOCK(&logmsgs);

	if (logthread != CSS_PTHREADT_NULL)
...
void css_log(int level, const char *file, int line, const char *function, const char *fmt, ...)
{
	...
	if (!(buf = css_str_thread_get(&log_buf, LOG_BUF_INIT_SIZE)))
		return;

	if (level != __LOG_VERBOSE && CSS_RWLIST_EMPTY(&logchannels)) {
		/*
		...
		int result;
		va_start(ap, fmt);
		result = css_str_set_va(&buf, BUFSIZ, fmt, ap); /* XXX BUFSIZ ? */
		va_end(ap);
		if (result != CSS_DYNSTR_BUILD_FAILED) {
			term_filter_escapes(css_str_buffer(buf));
			fputs(css_str_buffer(buf), stdout);
		}
		return;
	}
	...
	*/
	...
	/* Build string */
	va_start(ap, fmt);
	res = css_str_set_va(&buf, BUFSIZ, fmt, ap);
	va_end(ap);
	...
	if (!(logmsg = css_calloc_with_stringfields(1, struct logmsg, res + 128)))
		return;

	/* Copy string over */
	css_string_field_set(logmsg, message, css_str_buffer(buf));
	...
	css_localtime(&now, &tm, NULL);
	css_strftime(datestring, sizeof(datestring), dateformat, &tm);
	css_string_field_set(logmsg, date, datestring);

	/* Copy over data */
	logmsg->level = level;
	logmsg->line = line;
	css_string_field_set(logmsg, level_name, levels[level]);
	css_string_field_set(logmsg, file, file);
	css_string_field_set(logmsg, function, function);
	logmsg->process_id = (long) GETTID();
        
	/* If the logger thread is active, append it to the tail end of the list - otherwise skip that step */
	if (logthread != CSS_PTHREADT_NULL) {
		CSS_LIST_LOCK(&logmsgs);
		CSS_LIST_INSERT_TAIL(&logmsgs, logmsg, list);
		css_cond_signal(&logcond);
		CSS_LIST_UNLOCK(&logmsgs);
	} else {
...
{
	struct css_bt *bt = css_calloc(1, sizeof(*bt));
	if (!bt) {
		css_log(LOG_ERROR, "Unable to allocate memory for backtrace structure!\n");
		return NULL;
	}

	bt->alloced = 1;

	css_bt_get_addresses(bt);
...
{
	bt->num_frames = backtrace(bt->addresses, CSS_MAX_BT_FRAMES);
...
char **css_bt_get_symbols(void **addresses, size_t num_frames)
{
	...
	eachlen = css_calloc(num_frames, sizeof(*eachlen));

	if (!(strings = css_calloc(num_frames, sizeof(*strings)))) {
		return NULL;
	}
	...
	for (stackfr = 0; stackfr < num_frames; stackfr++) {
		...
		if (strcmp(dli.dli_fname, "ceictims") == 0) {
			char ceictimspath[256];
			if (!(dli.dli_fname = css_utils_which("ceictims", ceictimspath, sizeof(ceictimspath)))) {
				/* This will fail to find symbols */
				css_log(LOG_DEBUG, "Failed to find ceictims binary for debug symbols.\n");
				dli.dli_fname = "ceictims";
			}
		}

		lcssslash = strrchr(dli.dli_fname, '/');
		if (	(bfdobj = bfd_openr(dli.dli_fname, NULL)) &&
				bfd_check_format(bfdobj, bfd_object) &&
				(allocsize = bfd_get_symtab_upper_bound(bfdobj)) > 0 &&
				(syms = css_malloc(allocsize)) &&
			...
			for (section = bfdobj->sections; section; section = section->next) {
				...
				/* Stack trace output */
				found++;
				if ((lcssslash = strrchr(file, '/'))) {
					const char *prevslash;
					for (prevslash = lcssslash - 1; *prevslash != '/' && prevslash >= file; prevslash--);
					if (prevslash >= file) {
						lcssslash = prevslash;
					}
				}
				...
				snprintf(msg, sizeof(msg), "%s:%u %s()%s",
					lcssslash ? lcssslash + 1 : file, line,
					S_OR(func, "???"),
					address_str);
		...
		if (!found) {
			...
			snprintf(msg, sizeof(msg), "%s %s()%s",
				lcssslash ? lcssslash + 1 : dli.dli_fname,
				S_OR(dli.dli_sname, "<unknown>"),
				address_str);
		}

		if (!css_strlen_zero(msg)) {
			char **tmp;
			eachlen[stackfr] = strlen(msg);
			if (!(tmp = css_realloc(strings, strings_size + eachlen[stackfr] + 1))) {
				css_free(strings);
				strings = NULL;
			...
			css_copy_string(strings[stackfr], msg, eachlen[stackfr] + 1);
...
void css_backtrace(void)
{
	...
	if (!(bt = css_bt_create())) {
		css_log(LOG_WARNING, "Unable to allocate space for backtrace structure\n");
		return;
	}

	if ((strings = css_bt_get_symbols(bt->addresses, bt->num_frames))) {
		css_debug(1, "Got %d backtrace record%c\n", bt->num_frames, bt->num_frames != 1 ? 's' : ' ');
		for (i = 3; i < bt->num_frames - 2; i++) {
			css_log(LOG_DEBUG, "#%d: [%p] %s\n", i - 3, bt->addresses[i], strings[i]);
		}
...
#undef free
		...
		css_debug(1, "Could not allocate memory for backtrace\n");
	}
	css_bt_destroy(bt);
#else
	css_log(LOG_WARNING, "Must run configure with '--with-execinfo' for stack backtraces.\n");
#endif /* defined(HAVE_BKTR) */
}

void __css_verbose_ap(const char *file, int line, const char *func, const char *fmt, va_list ap)
{
	struct css_str *buf = NULL;
	int res = 0;


	if (!(buf = css_str_thread_get(&verbose_buf, VERBOSE_BUF_INIT_SIZE)))
		return;

	if (css_opt_timestamp) {
		struct timeval now;
		struct css_tm tm;
		char date[40];
		char *datefmt;
		...
		now = css_tvnow();
		css_localtime(&now, &tm, NULL);
		css_strftime(date, sizeof(date), dateformat, &tm);
	...
	css_log(__LOG_VERBOSE, file, line, func, "%s", css_str_buffer(buf));
}

void __css_verbose(const char *file, int line, const char *func, const char *fmt, ...)
{
	va_list ap;

	va_start(ap, fmt);
	__css_verbose_ap(file, line, func, fmt, ap);
	va_end(ap);
}

/* No new code should use this directly, but we have the ABI for backwards compat */
#undef css_verbose
void __attribute__((format(printf, 1,2))) css_verbose(const char *fmt, ...);
void css_verbose(const char *fmt, ...)
{
	va_list ap;

	va_start(ap, fmt);
	__css_verbose_ap("", 0, "", fmt, ap);
	va_end(ap);
}

int css_register_verbose(void (*v)(const char *string)) 
{
	struct verb *verb;

	if (!(verb = css_malloc(sizeof(*verb))))
		return -1;

	verb->verboser = v;

	CSS_RWLIST_WRLOCK(&verbosers);
	CSS_RWLIST_INSERT_HEAD(&verbosers, verb, list);
	CSS_RWLIST_UNLOCK(&verbosers);
	
	return 0;
}

int css_unregister_verbose(void (*v)(const char *string))
{
	struct verb *cur;

	CSS_RWLIST_WRLOCK(&verbosers);
	CSS_RWLIST_TRAVERSE_SAFE_BEGIN(&verbosers, cur, list) {
		if (cur->verboser == v) {
			CSS_RWLIST_REMOVE_CURRENT(list);
			css_free(cur);
			break;
		}
	}
...
static void update_logchannels(void)
{
	...
	CSS_RWLIST_TRAVERSE(&logchannels, cur, list) {
...
int css_logger_register_level(const char *name)
{
	...
	levels[available] = css_strdup(name);

	CSS_RWLIST_UNLOCK(&logchannels);

	css_debug(1, "Registered dynamic logger level '%s' with index %d.\n", name, available);
...
void css_logger_unregister_level(const char *name)
{
	...
	if (found) {
		...
		css_debug(1, "Unregistered dynamic logger level '%s' with index %d.\n", name, x);
