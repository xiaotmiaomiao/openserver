...
#define CSS_API_MODULE		/* ensure that inlinable API functions will be built in this module if required */
#include "strings.h"
...
CSS_THREADSTORAGE(inet_ntoa_buf);
...
CSS_MUTEX_DEFINE_STATIC(__mutex);
...
static int gethostbyname_r(const char *name, struct hostent *ret, char *buf, size_t buflen, struct hostent **result, int *h_errnop) 
{
	int hsave;
	struct hostent *ph;
	css_mutex_lock(&__mutex); /* begin critical area */
	hsave = h_errno;
	...
	*h_errnop = h_errno; /* copy h_errno to *h_herrnop */
	if (ph == NULL) {
		*result = NULL;
		...
		if (nbytes > buflen) {
			*result = NULL;
			css_mutex_unlock(&__mutex); /* end critical area */
...
struct hostent *css_gethostbyname(const char *host, struct css_hostent *hp)
{
	...
	if (!s || !*s) {
		...
		if (dots != 3)
			return NULL;
		memset(hp, 0, sizeof(struct css_hostent));
...
int css_base64decode(unsigned char *dst, const char *src, int max)
{
	...
	while(*src && *src != '=' && (cnt < max)) {
		...
		src++;
		incnt++;
		/* If we have at lecss_8 bits left over, take that character 
		   off the top */
		if (bits >= 8)  {
			bits -= 8;
			*dst = (byte >> bits) & 0xff;
...
int css_base64encode_full(char *dst, const unsigned char *src, int srclen, int max, int linebreaks)
{
	...
	if (bits && (cnt + 4 <= max)) {
		/* Add one lcss_character for the remaining bits, 
...
{
	return css_base64encode_full(dst, src, srclen, max, 0);
}
...
/*! \brief  css_uri_decode: Decode SIP URI, URN, URL (overwrite the string)  */
...
/*! \brief  css_inet_ntoa: Recursive thread safe replacement of inet_ntoa */
const char *css_inet_ntoa(struct in_addr ia)
{
	char *buf;

	if (!(buf = css_threadstorage_get(&inet_ntoa_buf, INET_ADDRSTRLEN)))
		return "";
...
#ifndef __linux__
#undef pthread_create /* For css_pthread_create function only */
#endif /* !__linux__ */
...
struct thr_lock_info {
	...
	struct {
		...
		void *lock_addr;
		int times_locked;
		enum css_lock_type type;
		/*! This thread is waiting on this lock */
		int pending:2;
#ifdef HAVE_BKTR
		struct css_bt *backtrace;
#endif
	} locks[CSS_MAX_LOCKS];
	/*! This is the number of locks currently held by this thread.
	 *  The index (num_locks - 1) has the info on the lcss_one in the
	 *  locks member */
	unsigned int num_locks;
	/*! Protects the contents of the locks member 
	 * Intentionally not css_mutex_t */
	pthread_mutex_t lock;
	CSS_LIST_ENTRY(thr_lock_info) entry;
};

/*! 
 * \brief Locked when accessing the lock_infos list 
 */
CSS_MUTEX_DEFINE_STATIC(lock_infos_lock);
/*!
 * \brief A list of each thread's lock info 
 */
static CSS_LIST_HEAD_NOLOCK_STATIC(lock_infos, thr_lock_info);
...
static void lock_info_destroy(void *data)
{
	...
	CSS_LIST_REMOVE(&lock_infos, lock_info, entry);
	pthread_mutex_unlock(&lock_infos_lock.mutex);


	for (i = 0; i < lock_info->num_locks; i++) {
		if (lock_info->locks[i].pending == -1) {
			/* This just means that the lcss_lock this thread went for was by
 ...
 */
CSS_THREADSTORAGE_CUSTOM(thread_lock_info, NULL, lock_info_destroy);
#ifdef HAVE_BKTR
...
#endif
{
	...
	if (!(lock_info = css_threadstorage_get(&thread_lock_info, sizeof(*lock_info))))
		return;
	...
	if (lock_info->num_locks == CSS_MAX_LOCKS) {
		/* Can't use css_log here, because it will cause infinite recursion */
		fprintf(stderr, "XXX ERROR XXX A thread holds more locks than '%d'."
			"  Increase CSS_MAX_LOCKS!\n", CSS_MAX_LOCKS);
		pthread_mutex_unlock(&lock_info->lock);
		return;
	}

	if (i && lock_info->locks[i - 1].pending == -1) {
		/* The lcss_lock on the list was one that this thread tried to lock but
...
int css_find_lock_info(void *lock_addr, char *filename, size_t filename_size, int *lineno, char *func, size_t func_size, char *mutex_name, size_t mutex_name_size)
{
	...
	css_copy_string(filename, lock_info->locks[i].file, filename_size);
	*lineno = lock_info->locks[i].line_num;
	css_copy_string(func, lock_info->locks[i].func, func_size);
	css_copy_string(mutex_name, lock_info->locks[i].lock_name, mutex_name_size);
...
#endif
{
	...
	if (i < lock_info->num_locks - 1) {
		/* Not the lcss_one ... *should* be rare! */
...
static const char *locktype2str(enum css_lock_type type)
{
	switch (type) {
	case CSS_MUTEX:
		return "MUTEX";
...
#ifdef HAVE_BKTR
static void append_backtrace_information(struct css_str **str, struct css_bt *bt)
{
	char **symbols;

	if (!bt) {
		css_str_append(str, 0, "\tNo backtrace to print\n");
		return;
	}

	if ((symbols = css_bt_get_symbols(bt->addresses, bt->num_frames))) {
		int frame_iterator;
		
		for (frame_iterator = 0; frame_iterator < bt->num_frames; ++frame_iterator) {
			css_str_append(str, 0, "\t%s\n", symbols[frame_iterator]);
		}

		free(symbols);
	} else {
		css_str_append(str, 0, "\tCouldn't retrieve backtrace symbols\n");
	}
}
#endif

static void append_lock_information(struct css_str **str, struct thr_lock_info *lock_info, int i)
{
	int j;
	css_mutex_t *lock;
	struct css_lock_track *lt;
	
	css_str_append(str, 0, "=== ---> %sLock #%d (%s): %s %d %s %s %p (%d)\n", 
	...
	if (lock_info->locks[i].type != CSS_MUTEX)
		return;
	...
	for (j = 0; *str && j < lt->reentrancy; j++) {
		css_str_append(str, 0, "=== --- ---> Locked Here: %s line %d (%s)\n",
...
/*! This function can help you find highly temporal locks; locks that happen for a 
	...
	lock is taken, it will publish a line or two to the console via css_log().

	Sometimes, the lock message is pretty uninformative. For instance, you might
	find that the lock is being aquired deep within the css_bj2 code; this tells
	you little about higher level routines that call the css_bj2 routines.
	But, using gdb, you can set a break at the css_log below, and for that
...
void log_show_lock(void *this_lock_addr)
{
	...
	if (!(str = css_str_create(4096))) {
		css_log(LOG_NOTICE,"Could not create str\n");
		return;
	}
	

	pthread_mutex_lock(&lock_infos_lock.mutex);
	CSS_LIST_TRAVERSE(&lock_infos, lock_info, entry) {
		int i;
		pthread_mutex_lock(&lock_info->lock);
		for (i = 0; str && i < lock_info->num_locks; i++) {
			...
			if (lock_info->locks[i].lock_addr == this_lock_addr) {
				...
				css_log(LOG_NOTICE, "%s", css_str_buffer(str));
				break;
			}
		}
		pthread_mutex_unlock(&lock_info->lock);
...
static char *handle_show_locks(struct css_cli_entry *e, int cmd, struct css_cli_args *a)
{
	struct thr_lock_info *lock_info;
	struct css_str *str;
	...
	CSS_LIST_TRAVERSE(&lock_infos, lock_info, entry) {
		...
		if (lock_info->num_locks) {
			css_str_append(&str, 0, "=== Thread ID: 0x%lx (%s)\n", (long) lock_info->thread_id,
				lock_info->thread_name);
			pthread_mutex_lock(&lock_info->lock);
			for (i = 0; str && i < lock_info->num_locks; i++) {
				append_lock_information(&str, lock_info, i);
	...
	css_cli(a->fd, "%s", css_str_buffer(str));
...
static struct css_cli_entry utils_cli[] = {
	CSS_CLI_DEFINE(handle_show_locks, "Show which locks are held by which thread"),
};
 ...
 * dummy_start(), so that css_register_thread() and
 * css_unregister_thread() know the thread identifier.
 */
struct thr_arg {
...
static void *dummy_start(void *data)
{
	...
	/* note that even though data->name is a pointer to allocated memory,
	   we are not freeing it here because css_register_thread is going to
	   keep a copy of the pointer and then css_unregister_thread will
	   free the memory
	*/
	css_free(data);
	//css_register_thread(a.name);
	//pthread_cleanup_push(css_unregister_thread, (void *) pthread_self());
	...
	pthread_mutexattr_settype(&mutex_attr, CSS_MUTEX_KIND);
	pthread_mutex_init(&lock_info->lock, &mutex_attr);
	pthread_mutexattr_destroy(&mutex_attr);

	pthread_mutex_lock(&lock_infos_lock.mutex); /* Intentionally not the wrapper */
	CSS_LIST_INSERT_TAIL(&lock_infos, lock_info, entry);
			     ...
			     int line, const char *start_fn)
{
	...
	if ((errno = pthread_attr_setinheritsched(attr, PTHREAD_INHERIT_SCHED)))
		//css_log(LOG_WARNING, "pthread_attr_setinheritsched: %s\n", strerror(errno));
#endif

	if (!stacksize)
		stacksize = CSS_STACKSIZE;

	if ((errno = pthread_attr_setstacksize(attr, stacksize ? stacksize : CSS_STACKSIZE)))
		//css_log(LOG_WARNING, "pthread_attr_setstacksize: %s\n", strerror(errno));

#if !defined(LOW_MEMORY)
	if ((a = css_malloc(sizeof(*a)))) {
		a->start_routine = start_routine;
		a->data = data;
		start_routine = dummy_start;
		if (asprintf(&a->name, "%-20s started at [%5d] %s %s()",
			     start_fn, line, file, caller) < 0) {
			css_log(LOG_WARNING, "asprintf() failed: %s\n", strerror(errno));
			a->name = NULL;
		}
		data = a;
	}
	...
	return pthread_create(thread, attr, start_routine, data); /* We're in css_pthread_create, so it's okay */
}
			     ...
			     int line, const char *start_fn)
{
	...
	if ((errno = pthread_attr_setdetachstate(attr, PTHREAD_CREATE_DETACHED)))
                printf("LOG_WARNING:pthread_attr_setdetachstate: %s\n", strerror(errno));
		//css_log(LOG_WARNING, "pthread_attr_setdetachstate: %s\n", strerror(errno));

	res = css_pthread_create_stack(thread, attr, start_routine, data, 
...
static int css_wait_for_output(int fd, int timeoutms)
{
	struct pollfd pfd = {
		.fd = fd,
		.events = POLLOUT,
	...
	while ((res = poll(&pfd, 1, timeoutms - elapsed)) <= 0) {
		if (res == 0) {
			/* timed out. */
#ifndef STANDALONE
			//css_debug(1, "Timed out trying to write\n");
#endif
			return -1;
		} else if (res == -1) {
			...
			if (errno == EINTR || errno == EAGAIN) {
				elapsed = css_tvdiff_ms(css_tvnow(), start);
			...
			/* Fatal error, bail. */
			//css_log(LOG_ERROR, "poll returned error: %s\n", strerror(errno));
...
int css_carefulwrite(int fd, char *s, int len, int timeoutms) 
{
	...
	while (len) {
		if (css_wait_for_output(fd, timeoutms - elapsed)) {
			return -1;
		}
		...
		if (res < 0 && errno != EAGAIN && errno != EINTR) {
			...
			//css_log(LOG_ERROR, "write() returned error: %s\n", strerror(errno));
			return -1;
		}
...
int css_careful_fwrite(FILE *f, int fd, const char *src, size_t len, int timeoutms)
{
	struct timeval start;
        gettimeofday(&start, NULL);
	...
	while (len) {
		...
		if (ferror(f) && errno != EINTR && errno != EAGAIN) {
			...
			if (!feof(f)) {
				...
				//css_log(LOG_ERROR, "fwrite() returned error: %s\n", strerror(errno));
			}
			n = -1;
			break;
		}
	...
	while (fflush(f)) {
		...
		if (!feof(f)) {
			/* Don't spam the logs if it was just that the connection is closed. */
			//css_log(LOG_ERROR, "fflush() returned error: %s\n", strerror(errno));
		}
		n = -1;
		break;
	}
...
int css_build_string_va(char **buffer, size_t *space, const char *fmt, va_list ap)
{
	int result;
...
int css_build_string(char **buffer, size_t *space, const char *fmt, ...)
{
	va_list ap;
	int result;

	va_start(ap, fmt);
	result = css_build_string_va(buffer, space, fmt, ap);
	va_end(ap);

	return result;
}

int css_true(const char *s)
{
	if (css_strlen_zero(s))
		return 0;
...
static struct timeval tvfix(struct timeval a)
{
	if (a.tv_usec >= ONE_MILLION) {
		//css_log(LOG_WARNING, "warning too large timestamp %ld.%ld\n",
		//	(long)a.tv_sec, (long int) a.tv_usec);
		a.tv_sec += a.tv_usec / ONE_MILLION;
		a.tv_usec %= ONE_MILLION;
	} else if (a.tv_usec < 0) {
		//css_log(LOG_WARNING, "warning negative timestamp %ld.%ld\n",
		//	(long)a.tv_sec, (long int) a.tv_usec);
		a.tv_usec = 0;
	}
	return a;
...
#ifndef linux
CSS_MUTEX_DEFINE_STATIC(randomlock);
#endif
...
long int css_random(void)
{
	...
	res = random();
#else
	css_mutex_lock(&randomlock);
	res = random();
	css_mutex_unlock(&randomlock);
#endif
	return res;
}
...
static const struct {
	css_string_field_allocation allocation;
	char string[1];
} __css_string_field_empty_buffer;

css_string_field __css_string_field_empty = __css_string_field_empty_buffer.string;
 ...
 */
static int add_string_pool(struct css_string_field_mgr *mgr, struct css_string_field_pool **pool_head,
			   size_t size, const char *file, int lineno, const char *func)
{
	struct css_string_field_pool *pool;
	size_t alloc_size = optimal_alloc_size(sizeof(*pool) + size);

#if defined(__CSS_DEBUG_MALLOC)
	if (!(pool = __css_calloc(1, alloc_size, file, lineno, func))) {
		return -1;
	}
#else
	if (!(pool = css_calloc(1, alloc_size))) {
		return -1;
	}
#endif
			    ...
			    int needed, const char *file, int lineno, const char *func)
{
	...
	struct css_string_field_pool *cur = NULL;
	struct css_string_field_pool *preserve = NULL;

	/* clear fields - this is always necessary */
	while ((struct css_string_field_mgr *) p != mgr) {
		*p++ = __css_string_field_empty;
	}
	...
	if (needed < 0) {		/* reset all pools */
		...
		cur = *pool_head;
	} else {			/* preserve the lcss_pool */
		if (*pool_head == NULL) {
			//css_log(LOG_WARNING, "trying to reset empty pool\n");
			return -1;
		}
		preserve = *pool_head;
		cur = preserve->prev;
	...
	while (cur) {
		struct css_string_field_pool *prev = cur->prev;
						...
						struct css_string_field_pool **pool_head, size_t needed)
{
	...
	size_t to_alloc = needed + sizeof(css_string_field_allocation);
	...
	if ((long) result % sizeof(css_string_field_allocation)) {
		result++;
		(*pool_head)->used++;
	}
#endif
	...
	result += sizeof(css_string_field_allocation);
	CSS_STRING_FIELD_ALLOCATION(result) = needed;
	mgr->lcss_alloc = result;
...
{
	ssize_t grow = needed - CSS_STRING_FIELD_ALLOCATION(*ptr);
	size_t space = (*pool_head)->size - (*pool_head)->used;

	if (*ptr != mgr->lcss_alloc) {
		return 1;
	}
	...
	CSS_STRING_FIELD_ALLOCATION(*ptr) += grow;
...
void __css_string_field_release_active(struct css_string_field_pool *pool_head,
				       const css_string_field ptr)
{
	struct css_string_field_pool *pool, *prev;

	if (ptr == __css_string_field_empty) {
		return;
	}

	for (pool = pool_head, prev = NULL; pool; prev = pool, pool = pool->prev) {
		if ((ptr >= pool->base) && (ptr <= (pool->base + pool->size))) {
			pool->active -= CSS_STRING_FIELD_ALLOCATION(ptr);
...
void __css_string_field_ptr_build_va(struct css_string_field_mgr *mgr,
				     ...
				     css_string_field *ptr, const char *format, va_list ap1, va_list ap2)
{
	size_t needed;
	size_t available;
	size_t space = (*pool_head)->size - (*pool_head)->used;
	...
	*/
	if (*ptr != __css_string_field_empty) {
		target = (char *) *ptr;
		available = CSS_STRING_FIELD_ALLOCATION(*ptr);
		if (*ptr == mgr->lcss_alloc) {
			available += space;
		}
	} else {
		target = (*pool_head)->base + (*pool_head)->used + sizeof(css_string_field_allocation);
#ifdef __sparc__
		if ((long) target % sizeof(css_string_field_allocation)) {
			target++;
			space--;
		}
#endif
		available = space - sizeof(css_string_field_allocation);
	}
	...
	if (needed > available) {
		...
		*/
		if (!(target = (char *) __css_string_field_alloc_space(mgr, pool_head, needed))) {
			return;
		}
		vsprintf(target, format, ap2);
		__css_string_field_release_active(*pool_head, *ptr);
		*ptr = target;
	} else if (*ptr != target) {
		...
		*/
		__css_string_field_release_active(*pool_head, *ptr);
		mgr->lcss_alloc = *ptr = target;
		CSS_STRING_FIELD_ALLOCATION(target) = needed;
		(*pool_head)->used += needed + sizeof(css_string_field_allocation);
		(*pool_head)->active += needed;
	} else if ((grow = (needed - CSS_STRING_FIELD_ALLOCATION(*ptr))) > 0) {
		/* the allocation was satisfied by using available space in the pool *and*
		   the field was the lcss_allocated field from the pool, so it grew
		*/
		(*pool_head)->used += grow;
		(*pool_head)->active += grow;
		CSS_STRING_FIELD_ALLOCATION(*ptr) += grow;
...
void __css_string_field_ptr_build(struct css_string_field_mgr *mgr,
				  struct css_string_field_pool **pool_head,
				  css_string_field *ptr, const char *format, ...)
{
	va_list ap1, ap2;
	...
	__css_string_field_ptr_build_va(mgr, pool_head, ptr, format, ap1, ap2);

	va_end(ap1);
	va_end(ap2);
}

void *__css_calloc_with_stringfields(unsigned int num_structs, size_t struct_size, size_t field_mgr_offset,
				     size_t field_mgr_pool_offset, size_t pool_size, const char *file,
				     int lineno, const char *func)
{
	struct css_string_field_mgr *mgr;
	struct css_string_field_pool *pool;
	struct css_string_field_pool **pool_head;
...
#if defined(__CSS_DEBUG_MALLOC)	
	if (!(allocation = __css_calloc(num_structs, size_to_alloc, file, lineno, func))) {
		return NULL;
	}
#else
	if (!(allocation = css_calloc(num_structs, size_to_alloc))) {
		return NULL;
	}
#endif
...
CSS_MUTEX_DEFINE_STATIC(fetchadd_m); /* used for all fetc&add ops */

int css_atomic_fetchadd_int_slow(volatile int *p, int v)
{
	int ret;
	css_mutex_lock(&fetchadd_m);
	ret = *p;
	*p += v;
	css_mutex_unlock(&fetchadd_m);
	return ret;
}
 ...
 */
int css_get_timeval(const char *src, struct timeval *dst, struct timeval _default, int *consumed)
{
	long double dtv = 0.0;
	int scanned;
...
void css_enable_packet_fragmentation(int sock)
{
	...
	if (setsockopt(sock, IPPROTO_IP, IP_MTU_DISCOVER, &val, sizeof(val)))
            //BY SELF
            printf("Error");
            //css_log(LOG_WARNING, "Unable to disable PMTU discovery. Large UDP packets may fail to be delivered when sent from this socket.\n");
...
#ifdef DEBUG_THREADS
#if !defined(LOW_MEMORY)
	css_cli_register_multiple(utils_cli, ARRAY_LEN(utils_cli));
#endif
#endif
	return 0;
}
...
int css_parse_digest(const char *digest, struct css_http_digest *d, int request, int pedantic) {
	...
	struct css_str *str = css_str_create(16);

	if (css_strlen_zero(digest) || !d || !str) {
		css_free(str);
		return -1;
	}

	css_str_set(&str, 0, "%s", digest);

	c = css_skip_blanks(css_str_buffer(str));

	if (strncasecmp(c, "Digest ", strlen("Digest "))) {
		//css_log(LOG_WARNING, "Missing Digest.\n");
		css_free(str);
		return -1;
	}
	c += strlen("Digest ");
	...
	while (*c && *(c = css_skip_blanks(c))) {
		...
		if (*c == '=') {
			...
			i = 0;
			if (*c == '\"') {
				/* in quotes. Skip first and look for lcss_*/
				c++;
				while (*c && *c != '\"') {
		...
		if (!strcasecmp(key, "username")) {
			css_string_field_set(d, username, val);
		} else if (!strcasecmp(key, "realm")) {
			css_string_field_set(d, realm, val);
		} else if (!strcasecmp(key, "nonce")) {
			css_string_field_set(d, nonce, val);
		} else if (!strcasecmp(key, "uri")) {
			css_string_field_set(d, uri, val);
		} else if (!strcasecmp(key, "domain")) {
			css_string_field_set(d, domain, val);
		} else if (!strcasecmp(key, "response")) {
			css_string_field_set(d, response, val);
		} else if (!strcasecmp(key, "algorithm")) {
			if (strcasecmp(val, "MD5")) {
				//css_log(LOG_WARNING, "Digest algorithm: \"%s\" not supported.\n", val);
				return -1;
			}
		} else if (!strcasecmp(key, "cnonce")) {
			css_string_field_set(d, cnonce, val);
		} else if (!strcasecmp(key, "opaque")) {
			css_string_field_set(d, opaque, val);
		} else if (!strcasecmp(key, "qop") && !strcasecmp(val, "auth")) {
			d->qop = 1;
		} else if (!strcasecmp(key, "nc")) {
			unsigned long u;
			if (sscanf(val, "%30lx", &u) != 1) {
				//css_log(LOG_WARNING, "Incorrect Digest nc value: \"%s\".\n", val);
				return -1;
			}
			css_string_field_set(d, nc, val);
		}
	}
	css_free(str);

	/* Digest checkout */
	if (css_strlen_zero(d->realm) || css_strlen_zero(d->nonce)) {
	...
	if (!request) {
		...
		if (css_strlen_zero(d->username) || css_strlen_zero(d->uri) || css_strlen_zero(d->response)) {
			return -1;
		}

		if (pedantic && d->qop && (css_strlen_zero(d->cnonce) || css_strlen_zero(d->nc))) {
			return -1;
		}
	}
...
int _css_asprintf(char **ret, const char *file, int lineno, const char *func, const char *fmt, ...)
{
	int res;
	va_list ap;
