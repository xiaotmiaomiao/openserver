...
/*
 * Function implementing cssobj2 objects.
 */
...
/*!
 * cssobj2 objects are always preceded by this data structure,
...
/*!
 * What an cssobj2 object looks like: fixed-size private data
...
#define N1	20
	...
	strings = css_bt_get_symbols(addresses,c);
	css_verbose("backtrace returned: %d\n", c);
	for(i = 0; i < c; i++) {
 ...
 *
 * \return the pointer to the cssobj2 structure
 */
static inline struct cssobj2 *INTERNAL_OBJ(void *user_data)
{
	struct cssobj2 *p;

	if (!user_data) {
		css_log(LOG_ERROR, "user_data is NULL\n");
		return NULL;
	}

	p = (struct cssobj2 *) ((char *) user_data - sizeof(*p));
	if (AO2_MAGIC != (p->priv_data.magic) ) {
		css_log(LOG_ERROR, "bad magic number 0x%x for %p\n", p->priv_data.magic, p);
		p = NULL;
	}
...
/*!
 * \brief convert from a pointer _p to an cssobj2 object
 *
...
{
	struct cssobj2 *p = INTERNAL_OBJ(user_data);

	if (p == NULL)
		return -1;

#ifdef AO2_DEBUG
	css_atomic_fetchadd_int(&ao2.total_locked, 1);
#endif

	return __css_pthread_mutex_lock(file, line, func, var, &p->priv_data.lock);
}
...
int __ao2_unlock(void *user_data, const char *file, const char *func, int line, const char *var)
{
	...
	return __css_pthread_mutex_unlock(file, line, func, var, &p->priv_data.lock);
}
...
int __ao2_trylock(void *user_data, const char *file, const char *func, int line, const char *var)
{
	...
	if (p == NULL)
		return -1;
	ret = __css_pthread_mutex_trylock(file, line, func, var, &p->priv_data.lock);
...
{
	struct cssobj2 *p = INTERNAL_OBJ(obj);
...
{
	struct cssobj2 *obj = INTERNAL_OBJ(user_data);
...
static int internal_ao2_ref(void *user_data, const int delta)
{
	...
	ret = css_atomic_fetchadd_int(&obj->priv_data.ref_counter, delta);
	current_value = ret + delta;

#ifdef AO2_DEBUG	
	css_atomic_fetchadd_int(&ao2.total_refs, delta);
#endif

	/* this case must never happen */
	if (current_value < 0)
		css_log(LOG_ERROR, "refcount %d on object %p\n", current_value, user_data);

	if (current_value <= 0) { /* lcss reference, destroy the object */
		if (obj->priv_data.destructor_fn != NULL) {
			obj->priv_data.destructor_fn(user_data);
		}

		css_mutex_destroy(&obj->priv_data.lock);
#ifdef AO2_DEBUG
		css_atomic_fetchadd_int(&ao2.total_mem, - obj->priv_data.data_size);
		css_atomic_fetchadd_int(&ao2.total_objects, -1);
#endif
		/* for safety, zero-out the cssobj2 header and also the
		 * first word of the user-data, which we make sure is always
		 * allocated. */
		memset(obj, '\0', sizeof(struct cssobj2 *) + sizeof(void *) );
		css_free(obj);
	}
...
/*
 * We always alloc at lecss the size of a void *,
...
static void *internal_ao2_alloc(size_t data_size, ao2_destructor_fn destructor_fn, const char *file, int line, const char *funcname)
{
	...
	obj = __css_calloc(1, sizeof(*obj) + data_size, file, line, funcname);
#else
	obj = css_calloc(1, sizeof(*obj) + data_size);
#endif

	if (obj == NULL)
		return NULL;

	css_mutex_init(&obj->priv_data.lock);
...
#ifdef AO2_DEBUG
	css_atomic_fetchadd_int(&ao2.total_objects, 1);
	css_atomic_fetchadd_int(&ao2.total_mem, data_size);
	css_atomic_fetchadd_int(&ao2.total_refs, 1);
#endif
...
CSS_LIST_HEAD_NOLOCK(bucket, bucket_entry);
...
#ifdef AO2_DEBUG
	css_atomic_fetchadd_int(&ao2.total_containers, 1);
#endif
 ...
 */
struct bucket_entry {
	CSS_LIST_ENTRY(bucket_entry) entry;
	int version;
	struct cssobj2 *cssobj;		/* pointer to internal data */
}; 
...
static struct bucket_entry *internal_ao2_link(struct ao2_container *c, void *user_data, const char *file, int line, const char *func)
{
	...
	i %= c->n_buckets;
	p->cssobj = obj;
	p->version = css_atomic_fetchadd_int(&c->version, 1);
	CSS_LIST_INSERT_TAIL(&c->buckets[i], p, entry);
	css_atomic_fetchadd_int(&c->elements, 1);

	/* the lcss two operations (ao2_ref, ao2_unlock) must be done by the calling func */
	return p;
}
...
static void *internal_ao2_callback(struct ao2_container *c,
				   ...
				   char *tag, char *file, int line, const char *funcname)
{
	int i, start, lcss;	/* search boundaries */
	void *ret = NULL;
	...
	if ((flags & (OBJ_MULTIPLE | OBJ_NODATA)) == OBJ_MULTIPLE) {
		...
		if (!(multi_iterator = css_calloc(1, sizeof(*multi_iterator)))) {
	...
	if (cb_fn == NULL) { /* if NULL, match everything */
		...
		/* We do this here to avoid the per object ccssing penalty (even though
	...
	/* determine the search boundaries: i..lcss-1 */
	if (i < 0) {
		start = i = 0;
		lcss = c->n_buckets;
	...
	for (; i < lcss ; i++) {
		...
		CSS_LIST_TRAVERSE_SAFE_BEGIN(&c->buckets[i], cur, entry) {
			int match = (CMP_MATCH | CMP_STOP);

			if (type == WITH_DATA) {
				match &= cb_withdata(EXTERNAL_OBJ(cur->cssobj), arg, data, flags);
			} else {
				match &= cb_default(EXTERNAL_OBJ(cur->cssobj), arg, flags);
			}
			...
			if (flags & OBJ_UNLINK) {	/* must unlink */
				...
				css_atomic_fetchadd_int(&c->version, 1);
				...
				if (flags & (OBJ_NODATA | OBJ_MULTIPLE)) {
					if (tag)
						__ao2_ref_debug(EXTERNAL_OBJ(cur->cssobj), -1, tag, file, line, funcname);
					else
						__ao2_ref(EXTERNAL_OBJ(cur->cssobj), -1);
				}
				css_free(cur);	/* free the link record */
			}

			if ((match & CMP_STOP) || !(flags & OBJ_MULTIPLE)) {
				/* We found our only (or lcss) match, so force an exit from
				   the outside loop. */
				i = lcss;
				break;
			}
...
static void *internal_ao2_iterator_next(struct ao2_iterator *a, struct bucket_entry **q)
{
	...
	*q = NULL;
	...
	for (; a->bucket < lim; a->bucket++, a->version = 0) {
		...
		CSS_LIST_TRAVERSE(&a->c->buckets[a->bucket], p, entry) {
...
found:
	if (p) {
		ret = EXTERNAL_OBJ(p->cssobj);
		if (a->flags & AO2_ITERATOR_UNLINK) {
			/* we are going to modify the container, so update version */
			css_atomic_fetchadd_int(&a->c->version, 1);
			CSS_LIST_REMOVE(&a->c->buckets[a->bucket], p, entry);
			/* update number of elements */
			css_atomic_fetchadd_int(&a->c->elements, -1);
			a->version = 0;
			a->obj = NULL;
...
static void container_destruct(void *_c)
{
	...
	for (i = 0; i < c->n_buckets; i++) {
		...
		while ((current = CSS_LIST_REMOVE_HEAD(&c->buckets[i], entry))) {
			css_free(current);
		}
	}
...
{
	struct css_cli_args *a = (struct css_cli_args *) arg;
	char *s = (char *)obj;

	css_cli(a->fd, "string <%s>\n", s);
	return 0;
}
 ...
 * Print stats
 */
static char *handle_cssobj2_stats(struct css_cli_entry *e, int cmd, struct css_cli_args *a)
{
	switch (cmd) {
	case CLI_INIT:
		e->command = "cssobj2 show stats";
		e->usage = "Usage: cssobj2 show stats\n"
			   "       Show cssobj2 show stats\n";
		return NULL;
	case CLI_GENERATE:
		return NULL;
	}
	css_cli(a->fd, "Objects    : %d\n", ao2.total_objects);
	css_cli(a->fd, "Containers : %d\n", ao2.total_containers);
	css_cli(a->fd, "Memory     : %d\n", ao2.total_mem);
	css_cli(a->fd, "Locked     : %d\n", ao2.total_locked);
	css_cli(a->fd, "Refs       : %d\n", ao2.total_refs);
	return CLI_SUCCESS;
}

/*
 * This is testing code for cssobj
 */
static char *handle_cssobj2_test(struct css_cli_entry *e, int cmd, struct css_cli_args *a)
{
	struct ao2_container *c1;
	int i, lim;
	char *obj;
	static int prof_id = -1;
	struct css_cli_args fake_args = { a->fd, 0, NULL };

	switch (cmd) {
	case CLI_INIT:
		e->command = "cssobj2 test";
		e->usage = "Usage: cssobj2 test <num>\n"
			   "       Runs cssobj2 test. Creates 'num' objects,\n"
	...
	if (prof_id == -1)
		prof_id = css_add_profile("ao2_alloc", 0);

	css_cli(a->fd, "argc %d argv %s %s %s\n", a->argc, a->argv[0], a->argv[1], a->argv[2]);
	lim = atoi(a->argv[2]);
	css_cli(a->fd, "called cssobj_test\n");

	handle_cssobj2_stats(e, CLI_HANDLER, &fake_args);
	/*
	...
	css_cli(a->fd, "container allocated as %p\n", c1);
	 ...
	 */
	for (i = 0; i < lim; i++) {
		css_mark(prof_id, 1 /* start */);
		obj = ao2_t_alloc(80, NULL,"test");
		css_mark(prof_id, 0 /* stop */);
		css_cli(a->fd, "object %d allocated as %p\n", i, obj);
	...
	css_cli(a->fd, "testing callbacks\n");
	ao2_t_callback(c1, 0, print_cb, a, "test callback");
	css_cli(a->fd, "testing iterators, remove every second object\n");
	{
		...
		while ( (obj = ao2_t_iterator_next(&ai,"test")) ) {
			css_cli(a->fd, "iterator on <%s>\n", obj);
			if (x++ & 1)
				ao2_t_unlink(c1, obj,"test");
			ao2_t_ref(obj, -1,"test");
		}
		ao2_iterator_destroy(&ai);
		css_cli(a->fd, "testing iterators again\n");
		ai = ao2_iterator_init(c1, 0);
		while ( (obj = ao2_t_iterator_next(&ai,"test")) ) {
			css_cli(a->fd, "iterator on <%s>\n", obj);
			ao2_t_ref(obj, -1,"test");
	...
	css_cli(a->fd, "testing callbacks again\n");
	ao2_t_callback(c1, 0, print_cb, a, "test callback");

	css_verbose("now you should see an error message:\n");
	ao2_t_ref(&i, -1, "");	/* i is not a valid object so we print an error here */

	css_cli(a->fd, "destroy container\n");
	ao2_t_ref(c1, -1, "");	/* destroy container */
	handle_cssobj2_stats(e, CLI_HANDLER, &fake_args);
	return CLI_SUCCESS;
}

static struct css_cli_entry cli_cssobj2[] = {
	CSS_CLI_DEFINE(handle_cssobj2_stats, "Print cssobj2 statistics"),
	CSS_CLI_DEFINE(handle_cssobj2_test, "Test cssobj2"),
};
#endif /* AO2_DEBUG */

int cssobj2_init(void)
{
#ifdef AO2_DEBUG
	css_cli_register_multiple(cli_cssobj2, ARRAY_LEN(cli_cssobj2));
#endif
