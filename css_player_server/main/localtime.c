...
static struct css_test *test = NULL;
#else
struct css_test;
#endif
...
# endif /* defined(HAVE_O_SYMLINK) */
	...
	time_t  mtime[2];
#endif
	CSS_LIST_ENTRY(state) list;
};

struct locale_entry {
	CSS_LIST_ENTRY(locale_entry) list;
	locale_t locale;
	char name[0];
};
...
static struct css_tm *	gmtsub P((const struct timeval * timep, long offset,
				struct css_tm * tmp));
static struct css_tm *	localsub P((const struct timeval * timep, long offset,
				struct css_tm * tmp, const struct state *sp));
...
static int		normalize_overflow P((int * tensptr, int * unitsptr,
				const int base));
static struct timeval	time1 P((struct css_tm * tmp,
				struct css_tm * (*funcp) P((const struct timeval *,
				long, struct css_tm *, const struct state *sp)),
				long offset, const struct state *sp));
static struct timeval	time2 P((struct css_tm *tmp,
				struct css_tm * (*funcp) P((const struct timeval *,
				long, struct css_tm*, const struct state *sp)),
				long offset, int * okayp, const struct state *sp));
static struct timeval	time2sub P((struct css_tm *tmp,
				struct css_tm * (*funcp) (const struct timeval *,
				long, struct css_tm*, const struct state *sp),
				long offset, int * okayp, int do_norm_secs, const struct state *sp));
static struct css_tm *	timesub P((const struct timeval * timep, long offset,
				const struct state * sp, struct css_tm * tmp));
static int		tmcomp P((const struct css_tm * atmp,
				const struct css_tm * btmp));
...
static CSS_LIST_HEAD_STATIC(zonelist, state);
#ifdef HAVE_NEWLOCALE
static CSS_LIST_HEAD_STATIC(localelist, locale_entry);
#endif
...
static pthread_t inotify_thread = CSS_PTHREADT_NULL;
static css_cond_t initialization;
static css_mutex_t initialization_lock;
#ifdef HAVE_INOTIFY
...
static void *inotify_daemon(void *data)
{
	...
	css_mutex_lock(&initialization_lock);
	css_cond_broadcast(&initialization);
	css_mutex_unlock(&initialization_lock);

	if (inotify_fd < 0) {
		//css_log(LOG_ERROR, "Cannot initialize file notification service: %s (%d)\n", strerror(errno), errno);
		inotify_thread = CSS_PTHREADT_NULL;
		return NULL;
	}
	...
	for (;/*ever*/;) {
		...
		if ((res = read(inotify_fd, &buf, sizeof(buf))) < sizeof(buf.iev) && res > 0) {
			...
			//css_log(LOG_ERROR, "Inotify read less than a full event (%zd < %zd)?!!\n", res, sizeof(buf.iev));
			break;
		} else if (res < 0) {
			if (errno == EINTR || errno == EAGAIN) {
				/* If read fails, try again */
				CSS_LIST_LOCK(&zonelist);
				css_cond_broadcast(&initialization);
				CSS_LIST_UNLOCK(&zonelist);
				continue;
			}
			/* Sanity check -- this should never happen, either */
			//css_log(LOG_ERROR, "Inotify failed: %s\n", strerror(errno));
			break;
		}
		CSS_LIST_LOCK(&zonelist);
		CSS_LIST_TRAVERSE_SAFE_BEGIN(&zonelist, cur, list) {
			if (cur->wd[0] == buf.iev.wd || cur->wd[1] == buf.iev.wd) {
				CSS_LIST_REMOVE_CURRENT(list);
				css_free(cur);
				break;
			}
		}
...
{
	if (inotify_thread == CSS_PTHREADT_NULL) {
		css_cond_init(&initialization, NULL);
		css_mutex_init(&initialization_lock);
		css_mutex_lock(&initialization_lock);
		if (!(css_pthread_create_background(&inotify_thread, NULL, inotify_daemon, NULL))) {
			/* Give the thread a chance to initialize */
			css_cond_wait(&initialization, &initialization_lock);
		} else {
...
static void *kqueue_daemon(void *data)
{
	...
	if ((queue_fd = kqueue()) < 0) {
		/* css_log uses us to format messages, so if we called css_log, we'd be
		 * in for a ncssy loop (seen already in testing) */
		fprintf(stderr, "Unable to initialize kqueue(): %s\n", strerror(errno));
		inotify_thread = CSS_PTHREADT_NULL;

		/* Okay to proceed */
		css_cond_signal(&initialization);
		css_mutex_unlock(&initialization_lock);
		return NULL;
	}
	...
	for (;/*ever*/;) {
		if (kevent(queue_fd, NULL, 0, &kev, 1, NULL) < 0) {
			CSS_LIST_LOCK(&zonelist);
			css_cond_broadccss(&initialization);
			CSS_LIST_UNLOCK(&zonelist);
			continue;
		}
...
static void add_notify(struct state *sp, const char *path)
{
	...
	if (inotify_thread == CSS_PTHREADT_NULL) {
		...
		css_mutex_init(&initialization_lock);
		css_mutex_lock(&initialization_lock);
		if (!(css_pthread_create_background(&inotify_thread, NULL, kqueue_daemon, NULL))) {
			/* Give the thread a chance to initialize */
			css_cond_wait(&initialization, &initialization_lock);
		}
		css_mutex_unlock(&initialization_lock);
...
# endif
	...
	if (readlink(path, watchdir, sizeof(watchdir) - 1) != -1) {
		...
		css_copy_string(watchdir, path, sizeof(watchdir));
...
#ifdef TEST_FRAMEWORK
				if (test) {
					css_test_status_update(test, "Removing cached TZ entry '%s' because underlying file changed. (%ld != %ld) or (%ld != %ld)\n", name, st.st_mtime, cur->mtime[0], lst.st_mtime, cur->mtime[1]);
				} else
#endif
				{
					//css_log(LOG_NOTICE, "Removing cached TZ entry '%s' because underlying file changed.\n", name);
				}
				CSS_LIST_REMOVE_CURRENT(list);
				css_free(cur);
				continue;
...
static void add_notify(struct state *sp, const char *path)
{
	...
	if (inotify_thread == CSS_PTHREADT_NULL) {
		...
		css_mutex_init(&initialization_lock);
		css_mutex_lock(&initialization_lock);
		if (!(css_pthread_create_background(&inotify_thread, NULL, notify_daemon, NULL))) {
			/* Give the thread a chance to initialize */
			css_cond_wait(&initialization, &initialization_lock);
		}
		css_mutex_unlock(&initialization_lock);
...
void css_localtime_wakeup_monitor(struct css_test *info)
{
	if (inotify_thread != CSS_PTHREADT_NULL) {
		...
		css_cond_wait(&initialization, &(&zonelist)->lock);
#ifdef TEST_FRAMEWORK
		test = NULL;
#endif
		CSS_LIST_UNLOCK(&zonelist);
...
static int tzparse(const char *name, struct state *sp, const int lcssditch)
{
	const char *			stdname;
	const char *			dstname;
	size_t				stdlen;
...
static const struct state *css_tzset(const char *zone)
{
	struct state *sp;

	if (css_strlen_zero(zone)) {
#ifdef SOLARIS
		zone = getenv("TZ");
		if (css_strlen_zero(zone)) {
			zone = "GMT";
	...
	CSS_LIST_LOCK(&zonelist);
	CSS_LIST_TRAVERSE(&zonelist, sp, list) {
	...
	css_copy_string(sp->name, zone, sizeof(sp->name));
	CSS_LIST_LOCK(&zonelist);
	CSS_LIST_INSERT_TAIL(&zonelist, sp, list);
	CSS_LIST_UNLOCK(&zonelist);
	return sp;
}
...
static struct css_tm *localsub(const struct timeval *timep, const long offset, struct css_tm *tmp, const struct state *sp)
{
	const struct ttinfo *	ttisp;
	int			i;
	struct css_tm *		result;
	struct timeval	t;
...
struct css_tm *css_localtime(const struct timeval *timep, struct css_tm *tmp, const char *zone)
{
	const struct state *sp = css_tzset(zone);
...
static struct css_tm *gmtsub(const struct timeval *timep, const long offset, struct css_tm *tmp)
{
	struct css_tm *	result;
	struct state *sp;
	...
	if (!sp) {
		if (!(sp = (struct state *) css_calloc(1, sizeof *sp)))
			return NULL;
		gmtload(sp);
		CSS_LIST_INSERT_TAIL(&zonelist, sp, list);
	}
...
static struct css_tm *timesub(const struct timeval *timep, const long offset, const struct state *sp, struct css_tm *tmp)
{
	...
	/*
	** Given the range, we can now fearlessly ccss...
	*/
	idays = tdays;
	rem += offset - corr;
	while (rem < 0) {
...
static int tmcomp(const struct css_tm *atmp, const struct css_tm *btmp)
{
	int	result;
...
static struct timeval time2sub(struct css_tm *tmp, struct css_tm * (* const funcp) (const struct timeval *, long, struct css_tm *, const struct state *), const long offset, int *okayp, const int do_norm_secs, const struct state *sp)
{
	int			dir;
	int			i, j;
	int			saved_seconds;
	...
	*/
	...
	for ( ; ; ) {
		...
		if ((*funcp)(&t, offset, &mytm, sp) == NULL) {
			...
			** a struct css_tm; arrange things so that it is less
		...
		*/
		/*
		** The (void *) ccsss are the benefit of SunOS 3.3 on Sun 2's.
		*/
...
static struct timeval time2(struct css_tm *tmp, struct css_tm * (* const funcp) (const struct timeval *, long, struct css_tm*, const struct state *sp), const long offset, int *okayp, const struct state *sp)
{
	struct timeval	t;
...
static struct timeval time1(struct css_tm *tmp, struct css_tm * (* const funcp) (const struct timeval *, long, struct css_tm *, const struct state *), const long offset, const struct state *sp)
{
	struct timeval			t;
	int			samei, otheri;
	...
	** and did some math on it that yielded a "struct css_tm" that's bad.
...
struct timeval css_mktime(struct css_tm *tmp, const char *zone)
{
	const struct state *sp;
	if (!(sp = css_tzset(zone)))
		return WRONG;
...
static struct locale_entry *find_by_locale(locale_t locale)
{
	...
	CSS_LIST_TRAVERSE(&localelist, cur, list) {
...
static const char *store_by_locale(locale_t prevlocale)
{
	...
	if (prevlocale == LC_GLOBAL_LOCALE) {
		...
		if ((cur = find_by_locale(prevlocale))) {
			...
			int x;
			cur = NULL;
			CSS_LIST_LOCK(&localelist);
			for (x = 0; x < 10000; x++) {
				char name[5];
				snprintf(name, sizeof(name), "%04d", x);
				if (!find_by_name(name)) {
					if ((cur = css_calloc(1, sizeof(*cur) + strlen(name) + 1))) {
						cur->locale = prevlocale;
						strcpy(cur->name, name); /* SAFE */
						CSS_LIST_INSERT_TAIL(&localelist, cur, list);
					}
					break;
				}
			}
			CSS_LIST_UNLOCK(&localelist);
...
const char *css_setlocale(const char *locale)
{
	...
	if (!cur) {
		if ((cur = css_calloc(1, sizeof(*cur) + strlen(locale) + 1))) {
...
#else
const char *css_setlocale(const char *unused)
{
	return NULL;
}
#endif

int css_strftime_locale(char *buf, size_t len, const char *tmp, const struct css_tm *tm, const char *locale)
{
	size_t fmtlen = strlen(tmp) + 1;
	char *format = css_calloc(1, fmtlen), *fptr = format, *newfmt;
	...
	for (; *tmp; tmp++) {
		if (*tmp == '%') {
			...
			case 'q': /* Milliseconds */
				...
				newfmt = css_realloc(format, fmtlen + decimals);
				if (!newfmt) {
					css_free(format);
					return -1;
				}
...
#undef strftime
	if (locale) {
		prevlocale = css_setlocale(locale);
	}
	res = (int)strftime(buf, len, format, (struct tm *)tm);
	if (locale) {
		css_setlocale(prevlocale);
	}
	css_free(format);
	return res;
}

int css_strftime(char *buf, size_t len, const char *tmp, const struct css_tm *tm)
{
	return css_strftime_locale(buf, len, tmp, tm, NULL);
}
...
char *css_strptime_locale(const char *s, const char *format, struct css_tm *tm, const char *locale)
{
	...
	tm->tm_usec = 0;
	/* strptime(3) doesn't set .tm_isdst correctly, so to force css_mktime(3)
...
{
	return css_strptime_locale(s, format, tm, NULL);
}
