 ...
 * Author: root
 *
 * Created on April 23, 2014, 2:13 AM
 */
...
/*!
 * \brief Types used in css_realtime_require_field
 */
typedef enum {
	RQ_INTEGER1,
	RQ_UINTEGER1,
...
struct css_variable {
	 ...
	 * \note File must be stuffed before name because of css_include_rename().
	 */
	char stuff[0];
};

typedef struct css_config *config_load_func(const char *database, const char *table, const char *configfile, struct css_config *config, struct css_flags flags, const char *suggested_include_file, const char *who_asked);
typedef struct css_variable *realtime_var_get(const char *database, const char *table, va_list ap);
typedef struct css_config *realtime_multi_get(const char *database, const char *table, va_list ap);
 ...
 * path is considered relative to CSS_CONFIG_DIR
 ...
 *
 * \return an css_config data structure on success
 ...
 * \retval NULL on error
 */
#define css_config_load(filename, flags)	css_config_load2(filename, CSS_MODULE, flags)
 ...
 */
void css_config_destroy(struct css_config *config);

/*!
 * \brief returns the root css_variable of a config
 *
 * \param config pointer to an css_config data structure
 ...
 *
 * \details
 * Somewhat similar in intent as the css_category_browse.
 * List variables of config file category
 *
 * \retval css_variable list on success
 ...
 *
 * \details
 * This is equivalent to css_variable_browse(), but more efficient if we
 * already have the struct css_category * (e.g. from css_category_get())
 */
 ...
 *
 * \note
 * Unlike the variables in css_config, the resulting list of variables
 * MUST be freed with css_variables_destroy() as there is no container.
 *
 * \note
 * The difference between these two calls is that css_load_realtime excludes
 * fields whose values are NULL, while css_load_realtime_all loads all columns.
 *
 * \note
 * You should use the constant SENTINEL to terminate arguments, in
 * order to preserve cross-platform compatibility.
 */
struct css_variable *css_load_realtime(const char *family, ...) attribute_sentinel;
struct css_variable *css_load_realtime_all(const char *family, ...) attribute_sentinel;
 ...
 * order to preserve cross-platform compatibility.
 */
int css_realtime_require_field(const char *family, ...) attribute_sentinel;
 ...
 * the css_load_realtime, this function can return more than one entry and
 * is thus stored inside a traditional css_config structure rather than
 * just returning a linked list of variables.
 *
 * \note You should use the constant SENTINEL to terminate arguments, in
 * order to preserve cross-platform compatibility.
 */
struct css_config *css_load_realtime_multientry(const char *family, ...) attribute_sentinel;
 ...
 * order to preserve cross-platform compatibility.
 */
int css_update_realtime(const char *family, const char *keyfield, const char *lookup, ...) attribute_sentinel;
 ...
 * order to preserve cross-platform compatibility.
 */
int css_update2_realtime(const char *family, ...) attribute_sentinel;
 ...
 * order to preserve cross-platform compatibility.
 */
int css_store_realtime(const char *family, ...) attribute_sentinel;
 ...
 * order to preserve cross-platform compatibility.
 */
int css_destroy_realtime(const char *family, const char *keyfield, const char *lookup, ...) attribute_sentinel;
 ...
 */
void css_variables_destroy(struct css_variable *var);

/*!
 * \brief Register config engine
 * \retval 1 Always
 */
int css_config_engine_register(struct css_config_engine *newconfig);
...
void css_category_append(struct css_config *config, struct css_category *cat);
...
#ifdef MALLOC_DEBUG
struct css_variable *_css_variable_new(const char *name, const char *value, const char *filename, const char *file, const char *function, int lineno);
#define css_variable_new(a, b, c) _css_variable_new(a, b, c, __FILE__, __PRETTY_FUNCTION__, __LINE__)
#else
...
int css_config_text_file_save(const char *filename, const struct css_config *cfg, const char *generator);
int config_text_file_save(const char *filename, const struct css_config *cfg, const char *generator) __attribute__((deprecated));

struct css_config *css_config_internal_load(const char *configfile, struct css_config *cfg, struct css_flags flags, const char *suggested_incl_file, const char *who_asked);

/*!
 * \brief Support code to parse config file arguments
 *
 * \details
 * The function css_parse_arg() provides a generic interface to parse
...
enum css_parse_flags {
	...
	/* Returns a struct css_sockaddr, with optional default value
	...
	/* Port handling, for css_sockaddr. accept/ignore/require/forbid
 ...
 * \param flags combination of css_parse_flags to specify the
 ...
 * \details
 * Examples of use:
 *	css_parse_arg("223", PARSE_INT32|PARSE_IN_RANGE,
 *		&a, -1000, 1000);
 *              returns 0, a = 223
 *	css_parse_arg("22345", PARSE_INT32|PARSE_IN_RANGE|PARSE_DEFAULT,
 *		&a, 9999, 10, 100);
 *              returns 1, a = 9999
 *      css_parse_arg("22345ssf", PARSE_UINT32|PARSE_IN_RANGE, &b, 10, 100);
 *		returns 1, b unchanged
 *      css_parse_arg("www.foo.biz:44", PARSE_INADDR, &sa);
 *		returns 0, sa contains address and port
 *      css_parse_arg("www.foo.biz", PARSE_INADDR|PARSE_PORT_REQUIRE, &sa);
 ...
 * Additional macros are present to run simple functions (e.g. css_copy_string)
 * or to pass arguments to css_parse_arg()
 *
 * As an example:
 ...
 * dynamic string
 */
#define	CV_BOOL(__x, __dst)	CV_F(__x, (__dst) = css_true(__val) )
#define CV_UINT(__x, __dst)	CV_F(__x, (__dst) = strtoul(__val, NULL, 0) )
#define CV_STR(__x, __dst)	CV_F(__x, css_copy_string(__dst, __val, sizeof(__dst)))
#define CV_DSTR(__x, __dst)	CV_F(__x, css_free(__dst); __dst = css_strdup(__val))
#define CV_STRFIELD(__x, __obj, __field) CV_F(__x, css_string_field_set(__obj, __field, __val))

/*! \brief Check if require type is an integer type */
CSS_INLINE_API(
int css_rq_is_int(require_type type),
{
	switch (type) {
	case RQ_INTEGER1:
	case RQ_UINTEGER1:
 ...
 * \param maxlen Length passed through to css_str_* functions
