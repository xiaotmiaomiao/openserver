 ...
 * Author: root
 *
 * Created on April 23, 2014, 6:53 PM
 */
...
\section CssObj2_UsageContainers USAGE - CONTAINERS
...
- \ref cssobj2.h All documentation for functions and data structures
...
defined in a source file, before the #include of cssobj2.h,
...
one extra argument that their ao2_xxx counterpart. The lcss argument in
...
Each of these calls will generate at lecss one line of output in /tmp/refs.
...
in debugging, because it is possible to have the cssobj2 system run it
multiple times on the same object, commonly fatal to ceictims.
...
static struct sip_pvt *dialog_ref_debug(struct sip_pvt *p, char *tag, const char *file, int line, const char *func)
{
	if (p)
		...
		css_log(LOG_ERROR, "Attempt to Ref a null pointer\n");
	return p;
}
...
before the #include "ceictims/cssobj2.h" line, and add a descriptive
tag to each call. Recompile, and run Ceictims, exit ceictims with
...
The above may seem cssronomically more work than it is worth to debug
...
after the lcss test, decrement the pointer to allow the object
 ...
 *
 * Allocates a struct cssobj2 with sufficient space for the
...
#elif defined(__CSS_DEBUG_MALLOC)
 ...
 *
 * We allocate space for a struct cssobj_container, struct container
...
enum ao2_iterator_flags {
	 ...
	 * cssobj2 API and should be freed by ao2_iterator_destroy().
	 */
